#!/usr/bin/env python3
"""Extract the FD003 valid-patch-mean cache beside the existing last-patch cache."""
...
OUTPUT_ROOT = BASE_CACHE / "patch_representation_ablation"
REPRESENTATIONS = ("last_valid_patch", "valid_patch_mean")
BATCH_SIZE = 24
...
def prepare_directories() -> None:
    for representation in REPRESENTATIONS:
        ...
        for split in ("train", "val", "test"):
            ...
            if representation == "last_valid_patch":
                hardlink(
...
def main() -> None:
    ...
    for split, dataset in splits.items():
        ...
        embeddings = np.lib.format.open_memmap(
            OUTPUT_ROOT / "valid_patch_mean" / f"{split}_embeddings.npy",
            mode="w+",
            dtype=np.float16,
        ...
        offset = 0
        for batch in tqdm(loader, desc=f"FD003 valid-patch mean {split}", unit="batch"):
            mean = encoder.embed_variants(batch["context"], ("valid_patch_mean",))["valid_patch_mean"]
            ...
            embeddings[region] = mean.numpy().astype(np.float16)
            offset += count
        if offset != size:
    ...
    manifest = {
        ...
        "representations": {
            "last_valid_patch": "existing FD003 cache hard-linked without modification",
