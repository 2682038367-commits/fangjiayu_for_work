#!/usr/bin/env python3
"""Select an FD003 patch representation by validation RMSE and test only the winner."""
...
ROOT = PROJECT_ROOT / "artifacts" / "FD003" / "patch_representation_ablation"
...
def main() -> None:
    if not (ROOT / "manifest.json").exists():
        raise RuntimeError("run extract_fd003_patch_representations.py first")
    rows = []
    ...
    selected_row["predictions"] = str(
        checkpoint_path(selected).with_suffix(".predictions.json").relative_to(PROJECT_ROOT)
    )
