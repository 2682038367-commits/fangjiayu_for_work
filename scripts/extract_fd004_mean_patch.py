#!/usr/bin/env python3
"""Extract a resumable FD004 valid-patch-mean Chronos-2 cache."""
...
def cache_is_complete(split: str, expected_size: int) -> bool:
    ...
    if int(metadata["size"]) != expected_size:
        return False
    return all(
        (CACHE_DIR / f"{split}_{key}.npy").exists()
        and np.load(CACHE_DIR / f"{split}_{key}.npy", mmap_mode="r").shape == shape
...
def verify_split(split: str, expected_size: int) -> dict[str, object]:
    ...
    if embeddings.shape != (expected_size, 21, 768) or embeddings.dtype != np.float16:
        raise RuntimeError(f"unexpected {split} embedding cache: {embeddings.shape}, {embeddings.dtype}")
...
def main() -> None:
    splits, normalizer, train_ids, val_ids = prepare_splits(DATA_DIR, "FD004", seed=SEED)
    ...
    if pending:
        ...
        for split in pending:
            print(f"extracting FD004 {split}: {len(splits[split])} samples")
        ...
        print("all FD004 cache splits already complete; skipping extraction")
