...
def batch_generator(data, time, batch_size, rng=None):
...
def discriminative_score_metrics (ori_data, generated_data, eval_seed=None, split_indices=None):
  ...
  if eval_seed is not None:
    ...
    tf1.set_random_seed(eval_seed)
  rng = np.random.RandomState(eval_seed) if eval_seed is not None else np.random

  # Basic Parameters
  no, seq_len, dim = np.asarray(ori_data).shape    
  if generated_data.shape[-1] == 21:
    generated_data =  generated_data[:,:,[1, 2, 3, 6, 7, 8, 10, 11, 12, 13, 14, 16, 19, 20]]
  ...
  def discriminator (x, t):
    ...
    with tf1.variable_scope("discriminator", reuse = tf1.AUTO_REUSE) as vs:
      initializer_seed = eval_seed if eval_seed is not None else None
      d_cell = tf1.nn.rnn_cell.GRUCell(
          num_units=hidden_dim,
          activation=tf.nn.tanh,
          kernel_initializer=tf1.glorot_uniform_initializer(seed=initializer_seed),
      ...
      dense_seed = None if eval_seed is None else eval_seed + 1
      y_hat_logit = tf1.layers.dense(
          d_last_states,
          1,
          activation=None,
          kernel_initializer=tf1.glorot_uniform_initializer(seed=dense_seed),
          bias_initializer=tf.zeros_initializer(),
      )
  ...
  sess = tf1.Session(config=session_config)
  ...
  if split_indices is None:
    ...
    real_train_idx, real_test_idx, gen_train_idx, gen_test_idx = split_indices
    train_x = [ori_data[i] for i in real_train_idx]
    test_x = [ori_data[i] for i in real_test_idx]
    train_t = [ori_time[i] for i in real_train_idx]
    test_t = [ori_time[i] for i in real_test_idx]
    train_x_hat = [generated_data[i] for i in gen_train_idx]
    test_x_hat = [generated_data[i] for i in gen_test_idx]
    train_t_hat = [generated_time[i] for i in gen_train_idx]
    test_t_hat = [generated_time[i] for i in gen_test_idx]
  ...
  for itt in tqdm(range(iterations), desc='training', total=iterations):
    ...
    # Batch setting
    X_mb, T_mb = batch_generator(train_x, train_t, batch_size, rng=rng)
    X_hat_mb, T_hat_mb = batch_generator(train_x_hat, train_t_hat, batch_size, rng=rng)
          
    # Train discriminator
    _, step_d_loss = sess.run([d_solver, d_loss], 
                              feed_dict={X: X_mb, T: T_mb, X_hat: X_hat_mb, T_hat: T_hat_mb})     
    if itt % 500 == 0:       
      print('Loss: {}'.format(step_d_loss))
  ...
  #return discriminative_score,fake_acc
