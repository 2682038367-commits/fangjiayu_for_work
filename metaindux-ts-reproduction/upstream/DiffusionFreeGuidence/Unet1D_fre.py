...
class UNet1D_fre(nn.Module):
    def __init__(
        ...
        frequency_mask_mode = 'hard_random_quantile',
        frequency_threshold_init = 0.25,
        frequency_mask_temperature = 0.1,
    ):
        super().__init__()
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim = time_dim, classes_emb_dim = classes_dim),
                Residual(PreNorm(dim_in, Adaptive_Spectral_Block_c(dim_in))),
                # Residual(PreNorm(dim_in, crossFrequencyAttention(length // 2**(ind)))),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim, classes_emb_dim = classes_dim),
                Residual(PreNorm(dim_out, Adaptive_Spectral_Block_t(
                    dim_out,
                    mask_mode=frequency_mask_mode,
                    threshold_init=frequency_threshold_init,
                    mask_temperature=frequency_mask_temperature,
                ))),
                Upsample(dim_out, dim_in) if not is_last else  nn.Conv1d(dim_out, dim_in, 3, padding = 1)
            ]))

        self.minusTrend = PreNorm(dim_out, synTemporalBlock(dim_out, length // 2**(len(in_out)-ind-1)))
...
if __name__ == '__main__':
    ...
    input_size = 18
    window_size = 48
    model = UNet1D_cross(dim = 32, dim_mults = (1, 2), cond_drop_prob = 0.2, channels = input_size, length = window_size)
