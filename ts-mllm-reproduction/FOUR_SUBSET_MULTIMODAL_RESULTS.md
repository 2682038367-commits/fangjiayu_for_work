...
> ⚠️ **历史结果，已被 [FIXED_MAIN_PROTOCOL_RESULTS.md](FIXED_MAIN_PROTOCOL_RESULTS.md) 取代。**
> 下表的时间基线（B时间）与完整 TMAF 在 **FD002/FD004 上采用旧「全局 Min-Max」归一化**，
> 导致时间分支退化接近常数（时间 43.90/44.77、TMAF 44.34/44.90）。
> 当前主方案（`main_protocol_v1.json`）已对 FD002/FD004 改用**「按工况 Min-Max」**：
>
> | 子集 | 归一化 | 时间基线 (batch128) | 完整 TMAF (batch128) |
> |---|---|---:|---:|
> | FD001 | 全局 Min-Max | 21.54 | 16.32 |
> | FD002 | 按工况 Min-Max | **18.72** | **17.09** |
> | FD003 | 全局 Min-Max | 34.67 | 23.43 |
> | FD004 | 按工况 Min-Max | **20.78** | **19.00** |
>
> **归一化不同不可直接比较**：此前「batch128 时间基线崩成 43–44」是旧全局归一化的产物，
> 不是 batch size 的独立效应。详见 `REPRODUCTION_CONCLUSION.md` §三·附。
...
「旧」= 本表原始结果（FD002/FD004 用**全局 Min-Max**，已废弃）；「新」= 当前协议
（FD002/FD004 用**按工况 Min-Max**）。测试 RMSE，单位 cycle。

**时间基线（纯时间模型）：**

| 子集 | 旧·全局 Min-Max | 新·当前协议 |
|---|---:|---:|
| FD001 | 21.54 | 21.54（单工况，不变） |
| FD002 | 43.90 | **18.72** |
| FD003 | 34.67 | 34.67（单工况，不变） |
| FD004 | 44.77 | **20.78** |

**完整 TMAF（全模态）：**

| 子集 | 旧·全局 Min-Max | 新·当前协议 | 论文 |
|---|---:|---:|---:|
| FD001 | 15.22¹ | 16.32¹ | 12.45 |
| FD002 | 44.34 | **17.09** | 14.22 |
| FD003 | 23.43 | 23.43 | 11.97 |
| FD004 | 44.90 | **19.00** | 15.94 |

¹ FD001 旧→新差异来自 **MAE 链路统一**（stride1 旧 MAE → stride50 统一 MAE），不是
  归一化；FD001 始终全局 Min-Max。FD003 无变化。

论文值依据本地 PDF 第 8 页表 III。预测 clip[0,125] 后计算、另存 raw（FD001 旧 TMAF
raw RMSE 15.2344）。MAE / Score 等完整指标见 `FIXED_MAIN_PROTOCOL_RESULTS.md`。

验证 RMSE 与最优 epoch（**均为旧·全局 Min-Max 值**，FD002/FD004 已由按工况版本取代）：
